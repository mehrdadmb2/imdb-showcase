...
// صفحه‌بندی مقاوم: فقط کارت‌های صفحه جاری وارد DOM می‌شوند.
let currentPage = 1;
let pageSize = 24;
...
async function loadMovies() {
    ...
    try {
        ...
        if (data.last_manual_update) {
            ...
            if (lastUpdate) {
                lastUpdate.textContent = `آخرین به‌روزرسانی دستی: ${lastManualUpdate}`;
            }
        }

        allMovies = Array.isArray(data.movies)
            ? data.movies.map(normalizeMovieRecord)
            : Array.isArray(data) ? data.map(normalizeMovieRecord) : [];

        console.log(`✅ ${allMovies.length} رکورد بارگذاری شد.`);
        ...
        console.error('❌ خطای بارگذاری داده:', e);
        if (loading) {
            loading.textContent = `❌ خطا در بارگذاری داده‌ها: ${e.message}`;
            loading.classList.add('active');
        }
    }
}
...
function normalizeMovieRecord(movie) {
    ...
    return {
        ...
        user_rating: Number(m.user_rating) || 0,
        ...
        season_number: Number(m.season_number) || 0,
        episode_number: Number(m.episode_number) || 0,
...
function renderMovies() {
    ...
    grid.innerHTML = pageMovies.map((m) => {
        const genres = m.genres && m.genres !== 'N/A'
            ? m.genres.split(',').map(g => g.trim()).filter(Boolean).slice(0, 3)
                .map(g => `<span class="movie-genre-tag">${escapeHtml(g)}</span>`).join('')
            : '';

        const poster = getLocalPosterUrl(m) || getSafeString(m.poster, '');
        const safeTitle = escapeHtml(m.title);
        const posterHtml = poster
            ? `<img class="movie-poster" src="${escapeHtml(poster)}" alt="${safeTitle}" loading="lazy" decoding="async" data-poster-id="${escapeHtml(m.imdb_id)}" />`
            : `<div class="movie-poster-placeholder">🎬</div>`;

        const typeBadge = m.title_type === 'TV Episode'
            ? '<div class="movie-type-badge">📺 قسمت</div>'
            : m.title_type === 'TV Series'
                ? '<div class="movie-type-badge">📺 سریال</div>'
                : '';

        const r = Number(m.user_rating) || 0;
        const ratingBadge = r >= 8
            ? `${ratingEmoji(r)} ${r >= 9 ? 'عالی' : 'خیلی خوب'}`
            : r >= 6 ? `${ratingEmoji(r)} خوب` : r > 0 ? `${ratingEmoji(r)}` : '';

        const ratedDate = getSafeString(m.date_rated, '')
            ? new Date(m.date_rated).toLocaleDateString('fa-IR')
            : '';

        return `<div class="movie-card" data-imdb-id="${escapeHtml(m.imdb_id)}" tabindex="0" role="button" aria-label="نمایش جزئیات ${safeTitle}">
            ${posterHtml}
    ...
    grid.querySelectorAll('img.movie-poster').forEach(img => {
        img.addEventListener('error', () => {
            const fallback = document.createElement('div');
            fallback.className = 'movie-poster-placeholder';
...
function getLocalPosterUrl(movie) {
    const local = getSafeString(movie.poster_local, '');
    if (!local) return '';
    if (/^https?:\/\//i.test(local) || local.startsWith('/')) return local;
    return local.startsWith('posters/') ? local : `posters/${local}`;
}

function updatePaginationControls() {
    const bar = document.getElementById('paginationBar');
    const info = document.getElementById('paginationSummary');
    ...
    info.textContent = `${start}–${end} از ${total}`;
    if (select) select.value = String(pageSize);

    [first, prev].forEach(btn => { if (btn) btn.disabled = currentPage === 1; });
    [next, last].forEach(btn => { if (btn) btn.disabled = currentPage === pages; });

    const wanted = new Set([1, pages, currentPage, currentPage - 1, currentPage + 1, currentPage - 2, currentPage + 2]);
    const visible = [...wanted].filter(n => n >= 1 && n <= pages).sort((a,b) => a-b);
    const out = [];
    let previous = 0;
    for (const page of visible) {
        if (previous && page - previous > 1) out.push('<span class="page-ellipsis">…</span>');
        out.push(`<button type="button" class="page-number ${page === currentPage ? 'active' : ''}" data-page="${page}">${page}</button>`);
        previous = page;
    }
    numbers.innerHTML = out.join('');
}

function goToPage(page) {
    const pages = Math.max(1, Math.ceil(filteredMovies.length / pageSize));
    const target = Math.min(Math.max(1, Number(page) || 1), pages);
    if (target === currentPage && document.getElementById('moviesGrid')?.children.length) return;
    currentPage = target;
    renderMovies();
    const grid = document.getElementById('moviesGrid');
    if (grid) window.scrollTo({ top: Math.max(0, grid.getBoundingClientRect().top + window.scrollY - 100), behavior: 'smooth' });
}
...
function updateStats() {
    ...
    document.getElementById('totalHours').textContent = totalHours ? `${totalHours}س ${totalRestMinutes ? totalRestMinutes + 'د' : ''}` : `${totalRestMinutes}د`;
...
function openModal(id) {
    const m = allMovies.find(x => String(x.imdb_id) === String(id));
    if (!m) return;
    const ov = document.getElementById('modalOverlay');
    if (!ov) return;

    const modalPoster = document.getElementById('modalPoster');
    const posterUrl = getLocalPosterUrl(m) || getSafeString(m.poster, '');
    modalPoster.src = posterUrl || '';
    modalPoster.style.display = posterUrl ? 'block' : 'none';
    ...
    document.getElementById('modalRuntime').textContent = formatRuntime(m.runtime);
