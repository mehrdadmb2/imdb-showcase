...
def is_fully_cached(record: dict[str, Any]) -> bool:
    ...
    if not updated:
        return False
    raw_omdb = record.get("raw_omdb") if isinstance(record.get("raw_omdb"), dict) else {}
    has_snapshot = bool(raw_omdb) or bool(record.get("omdb_found"))
    has_core = any(clean(record.get(key)) for key in ("plot", "actors", "writer", "runtime", "imdb_rating", "genres"))
    poster_url = clean(record.get("poster")) or clean(raw_omdb.get("Poster"))
    poster_known_good = bool(clean(record.get("poster_local")))
    poster_known_missing = not poster_url and clean(record.get("poster_source")) == "fallback"
    return has_snapshot and has_core and (poster_known_good or poster_known_missing)
...
def fetch_omdb(imdb_id: str, pool: KeyPool) -> tuple[dict[str, Any] | None, str]:
    attempts = 0
    last_error = "no-key"
    while attempts < max(1, len(pool.keys)):
        key = pool.next()
        if not key:
            break
        attempts += 1
        try:
            ...
            if payload.get("Response") == "True":
                return payload, "ok"
            message = clean(payload.get("Error")) or "omdb-failed"
            if "limit" in message.lower() or "request" in message.lower():
                pool.usage[key] = MAX_PER_KEY
            last_error = message
        except Exception as exc:
            pool.mark_used(key)
            last_error = f"request-error:{exc.__class__.__name__}"
...
def process_record(
    ...
    # We may still download a missing local poster from the already-known URL;
    # this never consumes an OMDb API request.
    if previous and is_fully_cached(previous) and not cache_due(previous, now):
        ...
        if not current.get("poster_local"):
            raw_omdb = current.get("raw_omdb") if isinstance(current.get("raw_omdb"), dict) else {}
            poster_url = clean(current.get("poster")) or clean(raw_omdb.get("Poster"))
            local, downloaded = download_poster(current["imdb_id"], poster_url, preserve_existing="")
            if local:
                current["poster_local"] = local
                current["poster_source"] = "local-cache"
                if downloaded:
                    current["poster_cached_at"] = iso(now)
